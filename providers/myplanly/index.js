...
(async () => {
  ...
  if (calendarRequestData && requestUrl) {
    ...
    dates.forEach((date) => {
      subjects.forEach(({ value, text }) => {
        params.set("date", date); // Set the current date in the request parameters
        params.set("calendar", value); // Set the current subject in the request parameters
        const modifiedPostData = params.toString();
        requests.push(
          page.evaluate(
            async (url, postData, subject, date, userLabel) => {
              ...
              return { date: date.toString(), subject, userLabel, response: await response.text() };
            },
            requestUrl,
            modifiedPostData,
            value,
    ...
    responses.forEach(({ date, response, userLabel }) => {
      const $ = cheerio.load(response);

      // Log the number of panel-body elements found
      const panelBodies = $(".panel-body").slice(1);
      console.log(`Date: ${date}, Number of panel bodies found: ${panelBodies.length}`);

      // Extract appointment data from HTML using Cheerio
      panelBodies.each((i, element) => {
        const panelBodyHtml = $(element).html();
        console.log(`Panel body HTML: ${panelBodyHtml}`);
        console.log(`Processing panel body #${i + 1} for date: ${date}`);

        const paragraphs = $(element).find("p").toArray().map(p => $(p).text().trim());
        console.log(`Paragraphs extracted:`, paragraphs);

        const timeText = $(element).find("h5").text().replace(/[a-zA-Z,]/g, "").replace(/[žŽčČšŠćĆđĐ]/g, "").trim();
        console.log(`Time text extracted: ${timeText}`);
        ...
        console.log(`Customer name: ${name}, Last name: ${lastName}`);
